...
from scholarly_handoff import build, validate, markdown
...
class Phase22Tests(unittest.TestCase):
    ...
    def test_valid_handoff(self):
        h = build(self.s, self.e, fresh_translation=True, translated_chunks=["chunk0001"])
        self.assertTrue(validate(h)["valid"])
        self.assertIn("does **not** independently translate", markdown(h))

    def test_hash_mutation_blocks_review(self):
        h = build(self.s, self.e)
        self.e.write_text("mutated", encoding="utf-8")
        self.assertIn("translation hash mismatch", validate(h)["errors"])

    def test_identical_source_translation_rejected(self):
        h = build(self.s, self.s)
        self.assertFalse(validate(h)["valid"])
